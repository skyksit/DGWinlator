...
- `applicationId` is `com.retrople` (same 12-byte length as `com.winlator`; all `.tzst` assets are binary-patched accordingly by `scripts/patch_tzst.py` in the root repo), so the fork **coexists** with stock Winlator.
- Bridge intent action `com.retrople.action.PLAY_GAME` and signature-level permission `com.retrople.permission.LAUNCH_GAME`.
